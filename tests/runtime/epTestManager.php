...
/**
     ...
     * not tested since it is done automatically
     * by ezpdo. Not forcing the queries
     *
     */
    function _testUpdateSchemaNoForce(){
        // get the manager
        $this->assertTrue($m = & $this->m);

        // set up strategy, upgrade
        $m->setConfigOption('update_strategy', 'sim');
        ...
        // get databse type
        $url = $m->getConfigOption('default_dsn');
        $dbtype = strtolower(substr($url, 0, strpos($url, '://')));
     ...
     * test the add/alter/drop fields feature, add table
     * not tested since it is done automatically
     * by ezpdo. Forcing the queries
     *
     */
    function _testUpdateSchemaForce(){
        // get the manager
        $this->assertTrue($m = & $this->m);
        ...
        // force everything
        $m->setConfigOption('force_update', true);
    ...
    function _allTests($dbal, $dbtype) {
        ...
        echo "  alter without force schema..";
        $this->_testUpdateSchemaNoForce();
        echo "done " . epNewLine();

        echo "  alter with force schema..";
        $this->_testUpdateSchemaNoForce();
        echo "done " . epNewLine();
